...
-- 添加缺失的字段 (只添加updated_at)
ALTER TABLE categories ADD COLUMN updated_at DATETIME DEFAULT NULL;
...
-- 更新现有分类数据，添加图标和slug (如果还没有设置)
UPDATE categories SET 
  icon = '💻',
  slug = 'tech',
  sort_order = 1
WHERE name = '技术' AND (icon IS NULL OR slug IS NULL);

UPDATE categories SET 
  icon = '🌱',
  slug = 'life',
  sort_order = 2
WHERE name = '生活' AND (icon IS NULL OR slug IS NULL);

UPDATE categories SET 
  icon = '📚',
  slug = 'reading',
  sort_order = 3
WHERE name = '读书' AND (icon IS NULL OR slug IS NULL);

UPDATE categories SET 
  icon = '✈️',
  slug = 'travel',
  sort_order = 4
WHERE name = '旅行' AND (icon IS NULL OR slug IS NULL);
...
ALTER TABLE tags ADD COLUMN updated_at DATETIME DEFAULT NULL;
